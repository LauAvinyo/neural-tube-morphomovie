...
import numpy as np
from vedo import (Arrow, Line, Mesh, Picture, Plotter, Points, Spline, Text2D,
                  grep, precision, printc, settings, show, vector)
...
if not args.gene_name:  # some automatic guessing of the gene name
    gene_name = "Unknown"
    fl = args.image.lower()
    if 'hoxa11' in fl: gene_name = "HOXA11"
    if 'hoxd13' in fl: gene_name = "HOXD13"
    if 'sox9' in fl: gene_name = "SOX9"
    if 'meis' in fl: gene_name = "MEIS"
    if 'smad4' in fl: gene_name = "SMAD4"
...
if not args.staging_file:  # some automatic picking of previous staging points
    sf = "output/" + gene_name.lower() + "/" + os.path.basename(
        args.image).split('.')[0] + '.txt'
    ...
    if evt.keypress == "c":  # CLEAR
        ...
        if os.path.isfile(limbstager_exe):
            ...
            with open(outfile, "w") as f:
                f.write(
                    f"gene_mapper {outfile}  u 1.0  0 0 0 0 {len(cpoints)}\n")
            ...
            if os.path.isfile(
                    limbstager_file):  # pick previously staged points if exist
                printc("INFO: Trying to use previous staging data in file:",
                       limbstager_file,
                       c='lg')
                import shutil
        ...
        if not len(result):
            printc("Error - Could not stage the limb, RESULT tag is missing",
                   c="r")
            return
        result = result[0]
...
plt = Plotter(bg="blackboard",
              title=f"gene_mapper v{version} - gene: {gene_name}")
...
if os.path.isfile(limbstager_file):  # pick previously staged points if exist
    printc("INFO: loading previously picked points from staging file:",
           limbstager_file,
           c='lg',
           italic=True)
    cpoints = [[float(mi[1]), float(mi[2]), 0]
               for mi in grep(limbstager_file, 'MEASURED')]
    update()
...
plt.interactive().remove_callback(id1).remove_callback(id2).remove_callback(
    id3)
...
if not os.path.isfile(meshfile):
    printc(f"FATAL: there is no mesh for stage {ageh}h in {limbs_dir}_meshed/",
           c="r")
    exit(0)
