...
class Morpher:
    ...
    def save_points(self, filepath):
        ...
        points_data = {
            'arrow_starts': self.arrow_starts,
            'arrow_stops': self.arrow_stops
        }
    ...
    def load_points(self, filepath):
        """Load the arrow start and stop points from a file using pickle."""
        with open(filepath, 'rb') as f:
            points_data = pickle.load(f)
            self.arrow_starts = points_data['arrow_starts']
            self.arrow_stops = points_data['arrow_stops']
    ...
    def onkeypress(self, evt):  ###################################### MORPH & GENERATE
        if evt.keypress == 'm': ##--------- morph mesh1 based on the existing arrows
            ...
            self.dottedln = None
            self.intermidiates = intermediates
...
if __name__ == "__main__":
    ...
    morpher.load_points('morpher_points.pkl')
    morpher.n = 10
    morpher.start()
